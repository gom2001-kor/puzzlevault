...
- `AdController`의 보상형/전면 광고는 기본적으로 비활성입니다. 2026-09-26 변경에서 페이지의 자동 AdSense 로드도 제거했고, 소유권 확인용 게시자 메타 태그와 `ads.txt`는 유지했습니다. 현재 코드에는 실제 광고를 요청하는 기본 경로가 없습니다.
...
2. 광고 공급자를 켜기 전에 운영 지역/대상에 맞는 개인정보 및 동의 설정을 점검합니다. 선택형 Analytics는 기본 꺼짐이며 하단 설정에서 허용한 뒤 다음 운영 페이지에서만 실행됩니다. 이 분석 설정은 Google 인증 광고 CMP를 대체하지 않습니다. `enabled: true`는 기술 설정이며 개인정보 동의를 대신하지 않습니다.
